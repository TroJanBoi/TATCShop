...
<div class="wrapper">
  ...
  <?php
    ...
    $old_data=mysqli_fetch_array($con->query("SELECT * FROM user WHERE id = '$selected_id'"));
    if(isset($_POST['add'])){
        $email=$_POST['email'];
        $pass=$_POST['password'];
        $name=$_POST['name'];
        $position=$_POST['position'];
        $upd_data=$con->query("UPDATE user SET email = '$email', password = '$pass', name = '$name', position = '$position' WHERE id = '$selected_id' ");
